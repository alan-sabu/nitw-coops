...
export default function PurchaseModelUpdateForm(props) {
  const {
    id: idProp,
    purchaseModel: purchaseModelModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = purchaseModelRecord
      ? { ...initialValues, ...purchaseModelRecord }
      : initialValues;
    setEmail(cleanValues.email);
    setRoll(cleanValues.roll);
    setRoom(cleanValues.room);
    setHostel(cleanValues.hostel);
    setSubtotal(cleanValues.subtotal);
    setTotal(cleanValues.total);
    setErrors({});
  };
  const [purchaseModelRecord, setPurchaseModelRecord] = React.useState(
    purchaseModelModelProp
  );
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(PurchaseModel, idProp)
        : purchaseModelModelProp;
      setPurchaseModelRecord(record);
    };
    queryData();
  }, [idProp, purchaseModelModelProp]);
  React.useEffect(resetStateValues, [purchaseModelRecord]);
  const validations = {
    email: [],
    roll: [{ type: "Required" }],
    room: [{ type: "Required" }],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            PurchaseModel.copyOf(purchaseModelRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "PurchaseModelUpdateForm")}
      {...rest}
    >
      <TextField
        label="Email"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || purchaseModelModelProp)}
