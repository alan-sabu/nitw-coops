...
export default function CartModelUpdateForm(props) {
  const {
    id: idProp,
    cartModel: cartModelModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = cartModelRecord
      ? { ...initialValues, ...cartModelRecord }
      : initialValues;
    setProduct_name(cleanValues.product_name);
    setProduct_price(cleanValues.product_price);
    setProduct_qty(cleanValues.product_qty);
    setProduct_subtotal(cleanValues.product_subtotal);
    setEmail(cleanValues.email);
    setErrors({});
  };
  const [cartModelRecord, setCartModelRecord] =
    React.useState(cartModelModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(CartModel, idProp)
        : cartModelModelProp;
      setCartModelRecord(record);
    };
    queryData();
  }, [idProp, cartModelModelProp]);
  React.useEffect(resetStateValues, [cartModelRecord]);
  const validations = {
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            CartModel.copyOf(cartModelRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "CartModelUpdateForm")}
      {...rest}
    >
      <TextField
        label="Product name"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || cartModelModelProp)}
